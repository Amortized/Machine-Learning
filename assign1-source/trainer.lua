...
function trainerBatch(model, step)
   ...
   function trainer:train(dataset, max_step)
	 ...
	 for j = 1,dataset:size() do
	    dw = dw*(j-1)/j + model:dw(dataset[j][1], dataset[j][2], dw)/j
	 end
...
--[[
	 -- Shuffle the dataset 
	      local shuffledDataset = {}
	      -- Generating random order
	      local rorder = torch.randperm(dataset:size())
	      for i = 1, dataset:size() do
		 shuffledDataset[ rorder[i] ] = { dataset[i][1], dataset[i][2] }
	      end
	 --
	 --]]
	 ...
	 local dw = torch.zeros(model.w:size())
         for j = 1,dataset:size() do
            --dw = model:dw(shuffledDataset[j][1], shuffledDataset[j][2])
...
--[[
	 ...
	 local eval = torch.sqrt(torch.sum(torch.pow(model.w,2)))
	 
	 
 	 if(math.abs(min - eval)  < 0.001) then
	        print "Match"
		count = count + 1 
	 else
	        count = 0
...
--	 if( min > torch.sqrt(torch.sum(torch.pow(model.w,2))) ) then
	   min = torch.sqrt(torch.sum(torch.pow(model.w,2)))
	   --model.w = returnModel
--	 end
